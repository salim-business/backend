    ...
    Presentation.prototype = $.extend({}, Presentation.prototype, {
        load: function() {
            ...
            $.each(this.settings.actions, function(i, v) {
                let $button = _.dom.$presentation.find('[data-js-action="' + this + '"]');
                let cookie = theme.Cookie.get('presentation-' + this);

                if(cookie === 'enabled' && $button.attr('data-invert') !== 'true' && !$button.is(':checked')) {
                    toggleState(v, true);
                    $button.prop('checked', 'checked');
                } else if(cookie === 'enabled' && $button.attr('data-invert') === 'true' && $button.is(':checked')) {
                    toggleState(v, false);
